...
void logAssertionFailed(std::string_view, const std::source_location&, std::string msg) noexcept;
...
static void printAssertFailed(std::string_view condition, std::string_view message,
    ...
    if constexpr (sizeof...(args) > 0) {
        formatted_message = fmt::vformat(message, fmt::make_format_args(args...));
    } else {
