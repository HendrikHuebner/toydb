...
enum struct AccessError : char { OK = 0, DOES_NOT_EXIST, OUT_OF_BOUNDS };

enum struct IndexDataType : char { NONE = 0, BTREE };
    ...
    struct Data {
        ...
        */
        Data(char* data_) : header(*reinterpret_cast<Header*>(data_)) {
            ...
            auto columnsPtr = reinterpret_cast<DataType*>(data_);
            columns = std::vector<DataType>{columnsPtr, columnsPtr + header.columnCount};

            if (header.index != IndexDataType::NONE) {
                data = data_ + sizeof(DataType) * header.columnCount;
